...
| `npm run test-battery docker` (2026-08-13) | **16 PASS · 1 FAIL · 1 SKIP** — the FAIL is G16 (wizard 1803 has no Cancel after Next on this database; confirmed pre-existing by re-running the same probe against the previous code). `bc_switch_company` passes for the first time: it now re-opens the session on the company (see CLAUDE.md "Company Switching") |
| `npm run test-battery saas` (2026-08-13) | **16 PASS · 1 FAIL · 1 SKIP** — the FAIL is G5 (report download on SaaS), unchanged. Includes a confirmed company switch and a dialog write on a fresh `Dev` sandbox |
...
| **G15** | **A dialog button cannot be pressed BY NAME.** `bc_write_data`/`bc_read_data`/`bc_execute_action` now reach an open modal through section `dialog` (bc-saas F-4), but answering it is still limited to the six `bc_respond_dialog` responses. The dialogs measured here (report request pages) publish NO action nodes in their control tree, so there was nothing to name; a dialog that does publish them should already work via `bc_execute_action { section: "dialog", action }`. | live probe on devel1 + SaaS | S | Use `bc_respond_dialog` (ok / cancel / yes / no / abort / close). |
| **G16** | **`bc_wizard_navigate` cannot cancel wizard 1803 after Next** — "No wizard action of type 'cancel'". BC publishes back/next/finish only from step 2 on that page, so this may be correct behaviour rather than a bug; what is missing is telling the two apart. NOT a regression: reproduced identically against the pre-change code. | docker battery | S | `bc_close_page`, or `finish`. |
| **G17** | **`dismissTeachingTips` is implemented but unverified live.** BC's "About this page" callout covered the bottom-left corner of every manual image (bc-saas F-7). The dismissal is written and harmless (a clean capture stays clean), but neither `devel1` nor the SaaS sandbox showed a teaching tip to prove it closes one. | [screenshot-service.ts](../src/services/screenshot-service.ts) `dismissTeachingTips` | S | Verify on a page that has `AboutTitle`/`AboutText` (the DeCa pages do). |
| **G18** | **The expired-browser-session path is proven only forwards.** `refreshCookieJar()` was verified live on SaaS (fresh jar in ~2s, WS untouched, capture works after it), but a genuinely EXPIRED jar was not reproduced — that needs waiting out the ticket. | [aad-browser-provider.ts](../src/connection/auth/aad-browser-provider.ts) | S | If a capture ever reports "still on the Entra sign-in form", run `npm run login:aad`. |
