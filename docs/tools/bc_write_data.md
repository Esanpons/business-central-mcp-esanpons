...
- Use it to set editable header/card fields (`{ "Name": "Contoso", "City": "London" }`), FactBox fields (via `section`), line-item cells (`section: "lines"` with `rowIndex` or `bookmark`), or **the fields of an open modal dialog** (`section: "dialog"`).
...
| `section` | `string` | No | Section to write to (e.g., `"lines"` for document line items, `"factbox:Sales Addresses"` for a FactBox, `"dialog"` for the modal currently open over the page). Omit for header fields. |
...
- **Writing into a dialog.** BC gates many actions behind a modal that carries its own fields (`PageType = StandardDialog`: a reason, a comment, a posting date). That dialog is a DIFFERENT FORM from the page, so a write aimed at the page cannot see its controls no matter which key you use — not even the exact `controlPath` the action's own response just handed you. While it is open it is listed as the section `dialog` (a second concurrent one is `dialog#2`), with its fields and their `controlPath`s, and writing to it works exactly like any other section, `changed` / `reason` / `validationMessage` included. Fill it first, then answer with [bc_respond_dialog](./bc_respond_dialog.md). If you name a dialog field without a `section`, the error tells you which section to use instead of a bare "Field not found".
