# bc_build_manual
> Builds a step-by-step Business Central USER MANUAL as Markdown and/or a printable A4 web page, capturing an annotated screenshot per step.

> **Deciding what to produce?** Start with the [documentation guide](../guides/documenting.md) —
> it is the decision table for screenshot vs manual, `md` vs `html`, and how to get a PDF.
...
One authoring model, two outputs:
...
No PDF or DOCX is produced. The HTML *is* the print path.
...
Choose `formats: ["html"]` whenever the manual is meant to be *read*, handed to an end user, or printed; add `"md"` when it must also live in a repo.
...
| `formats` | array of `'md' \| 'html'` | No | Output formats. Defaults to `["md"]`. Pass both to get both. |
...
| `lang` | string | No | HTML only. Language of the generated chrome (cover kicker, index title, print button, hint): `ca` (default), `es`, `en`. Anything else falls back to English. The step text is whatever you write. |
| `cover` | boolean | No | HTML only. Emit the cover sheet. Default `true`. |
| `toc` | boolean | No | HTML only. Emit the index sheet. Default: only when the manual has 4 or more steps. |
...
## Prose formatting
`intro` and every step `body` are written in a small Markdown subset, rendered identically in both outputs (in `md` it is passed through verbatim; in `html` it is converted). Everything is HTML-escaped first, so prose can never inject markup into the page.
...
Files are written under the resolved output directory: `outDir` if given (absolute, else relative to `BC_MANUAL_DIR`), otherwise `BC_MANUAL_DIR` itself (default `./manuals`, resolved against the server's working directory). The document files are named `<slug>.{md,html,css,js}` where `<slug>` is the slugified `name` or `title`; per-step captures are written to a sibling `<slug>-img/` folder as `step-<n>.png`.
...
- A step heading and its screenshot share a `data-group`, so they are **never split** across sheets; a group that does not fit on the current sheet moves whole to the next one.
...
## Notes & limitations
- `formats` defaults to `["md"]` when omitted or empty. Ask for `"html"` explicitly when the manual is meant to be printed or handed over.
- `assets`, `lang`, `cover` and `toc` only affect the HTML output; they are ignored when only `md` is requested.
...
- Layout regressions are caught by `npm run verify:manual-html`, which builds a synthetic multi-page manual from PNGs on disk, paginates it in a real browser, and asserts no sheet overflows, every step is placed, the index resolves, and printing yields exactly one page per sheet.
