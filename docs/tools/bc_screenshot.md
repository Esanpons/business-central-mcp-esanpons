...
| `clickBeforeCapture` | string[] | No | Captions of controls to CLICK before capturing, in order (e.g. `["Lines"]` to open a document line grid, or a tab name). The deterministic companion to `expand`: use it when a section only reveals its content on an explicit toggle. Matched by visible text or aria-label, exact then prefix, across every frame. |
